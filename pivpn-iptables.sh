...
# Allow SSH traffic for VPN client 10.0.50.2
iptables -A INPUT -s 10.0.1.2 -p tcp --dport 22 -j ACCEPT

# Allow SSH traffic for devices on the 192.168.21.0/24 network
...
# Block SSH traffic for all other VPN clients
iptables -A INPUT -s "$VPN_SUBNET" -p tcp --dport 22 -j DROP
...
# uncomment the rules below if Pi-hole DNS is on the same host
# iptables -A INPUT -i wg0 -p udp --dport 53 -j ACCEPT
# iptables -A INPUT -i wg0 -p tcp --dport 53 -j ACCEPT

# Allow VPN clients to query external DNS servers
iptables -A FORWARD -s "$VPN_SUBNET" -d 192.168.1.4 -p udp --dport 53 -j ACCEPT
iptables -A FORWARD -s "$VPN_SUBNET" -d 192.168.1.5 -p udp --dport 53 -j ACCEPT
iptables -A FORWARD -s "$VPN_SUBNET" -d 192.168.1.4 -p tcp --dport 53 -j ACCEPT
iptables -A FORWARD -s "$VPN_SUBNET" -d 192.168.1.5 -p tcp --dport 53 -j ACCEPT

# Allow HTTP traffic from the local network (192.168.x.x)
# iptables -A INPUT -s 192.168.0.0/16 -p tcp --dport 80 -j ACCEPT

# Allow HTTPs traffic from the lcoal netowrk (192.168.x.x)
# iptables -A INPUT -s 192.168.0.0/16 -p tcp --dport 443 -j ACCEPT

# Allow DNS traffic (TCP and UDP) from localhost (127.0.0.1)
# iptables -A INPUT -s 127.0.0.0/8 -p tcp --dport 53 -j ACCEPT
# iptables -A INPUT -s 127.0.0.0/8 -p udp --dport 53 -j ACCEPT

# Allow DNS traffic (TCP and UDP) from the local network (192.168.x.x)
...
# Allow Pi-hole API and web interface traffic on port 4711 from localhost
# iptables -A INPUT -p tcp --dport 4711 -i lo -j ACCEPT

# WireGuard: Allow Specific Clients to Access Intranet
iptables -A FORWARD -s 10.0.1.2 -d 192.168.0.0/16 -j ACCEPT  # Allow client 10.0.50.2 access to intranet
iptables -A FORWARD -s "$VPN_SUBNET" -d 192.168.0.0/16 -j DROP  # Block all other VPN clients from intranet access

# WireGuard: Allow All Clients to Access the Internet
iptables -A FORWARD -s "$VPN_SUBNET" -i wg0 -o eth0 -j ACCEPT
iptables -A FORWARD -d "$VPN_SUBNET" -i eth0 -o wg0 -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT
