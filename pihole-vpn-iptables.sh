...
# WireGuard VPN and DNS traffic
iptables -A INPUT -i eth0 -p udp --dport 51822 -m comment --comment "WireGuard VPN traffic" -j ACCEPT
...
iptables -A FORWARD -s 10.0.1.2 -d 192.168.0.0/16 -j ACCEPT  # Allow client 10.0.1.2 access to intranet
#Add a new rule specifically allowing client 10.0.1.x to access the intranet.
# Place this rule before the DROP rule that blocks other clients from accessing the intrane
